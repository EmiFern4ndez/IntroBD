...
int agregarPersona(char nomArch[15]) {
    FILE *arch;
    Persona p;
...
int modificarNacimiento(char nomArch[15], int dni){
    FILE *arch;
    Persona p;
    arch = fopen(nomArch, "rb+");
    int ok = 0;
    if (arch != NULL){
        while(!(feof(arch)) && (ok != 1)){
...
int main(int argc, char const *argv[]){
    ...
    printf("d) Agregar una persona\n");
    printf("e) Modificar fecha de nacimiento\n");
    printf("x) Para salir\n");
    scanf(" %c", &opciones);
    ...
    while(opciones != 'x'){
        ...
        case 'e':{
            int dni;
            printf("Ingrese el dni de la persona que quieres modificarle la fecha de nacimiento\n");
            scanf("%d", &dni);
            if (modificarNacimiento(nomArch, dni)){
                printf("Se modifico con exito el nacimiento de la persona con DNI: %d", dni);
            }else   
                printf("No se pudo modificar la fecha de nacimiento\n");
            break;
        }
